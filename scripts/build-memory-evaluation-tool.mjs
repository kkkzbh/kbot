...
const TOOLS = [
  {
    entry: resolve(ROOT_DIR, 'src/tools/memory-evaluation.ts'),
    outputName: 'memory-evaluation.mjs',
  },
  {
    entry: resolve(ROOT_DIR, 'src/tools/memory-evaluation-adapter.ts'),
    outputName: 'memory-evaluation-adapter.mjs',
  },
];
const OWNER_MARKER = '.memory-evaluation-tool-output.json';
const OWNER_MARKER_CONTENT = `${JSON.stringify({
  owner: 'qqbot-memory-evaluation-tool',
  schemaVersion: 1,
})}\n`;
...
async function assertOwnedOutput(outDir) {
  ...
  if (await pathExists(markerPath)) {
    ...
    const unownedOutput = (await Promise.all(outputPaths.map(async (path) => (
      (await pathExists(path)) ? path : null
    )))).find((path) => path !== null);
    if (unownedOutput) {
      throw new Error(`Refusing to replace an unowned output file: ${unownedOutput}`);
    }
...
async function main() {
  ...
  const temporaryDir = await mkdtemp(
    resolve(dirname(outDir), '.memory-evaluation-build-'),
  );
  try {
    ...
    for (const tool of TOOLS) {
      ...
      if (!stagedInfo.isFile() || stagedInfo.isSymbolicLink()) {
        throw new Error(`Built memory evaluation artifact is invalid: ${stagedOutput}`);
      }
  ...
  process.stdout.write(`[info] Memory evaluation tool built: ${outDir}\n`);
}
