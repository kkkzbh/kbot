...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (argument !== '--out-dir') throw new Error(`Unknown argument: ${argument}`);
  ...
  try {
    await build({
      ...
      build: {
        ...
        sourcemap: false,
        rollupOptions: {
          external: [/^node:/, /^koishi-plugin-chatluna(?:\/|$)/],
          output: {
            format: 'es',
    ...
    if (!stagedInfo.isFile() || stagedInfo.isSymbolicLink()) {
      throw new Error(`Built memory cutover artifact is invalid: ${stagedOutput}`);
    }
