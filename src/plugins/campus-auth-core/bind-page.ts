...
export function renderCampusBindPage(options: CampusBindPageOptions): string {
  ...
  const body = options.state === 'form'
    ? `${options.message ? `<p class="notice error">${escapeHtml(options.message)}</p>` : ''}<p class="lead">请选择绑定方式。敏感信息只会提交到 QQBot 绑定服务。</p>
       ${renderAppBridge(options.appBridge)}
...
function renderAppBridge(appBridge: CampusBindPageOptions['appBridge']): string {
  if (!appBridge) return '';
  return `<section class="app-bridge"><img src="${escapeHtml(appBridge.qrImageDataUrl)}" alt="志愿汇 App 授权二维码"><div><h2>${escapeHtml(appBridge.label)}</h2><p>${escapeHtml(appBridge.description)}</p><strong>请用志愿汇 App 首页“扫一扫”扫描左侧二维码。</strong></div></section>`;
}
