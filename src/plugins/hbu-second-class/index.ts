  ...
  ctx.middleware(async (session, next) => {
    ...
    try {
      ...
      if (command.kind === 'menu') {
        ...
        const status = await services.campusAuth.getStatus(identity.ownerKey, CAMPUS_AUTH_PROVIDER_SECOND_CLASS);
...
export function shouldExposeSecondClassCapabilityReference(session: Session): boolean {
  ...
  if (/^二课(?:确认\d{6}|成绩单\S+)/.test(text)) return true;
...
async function reply(
  ...
  await nativeFeatureChat.sendReply(session, {
    ...
    userText: command.kind === 'confirm' ? '二课确认 <确认码已隐藏>' : text,
    reply: content,
    summary,
    success,
    includeReplyPayload,
