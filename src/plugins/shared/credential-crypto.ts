...
export interface CredentialKek {
  id: string;
  key: Buffer;
}
...
export function loadOrCreateKek(path: string): CredentialKek {
  if (!existsSync(path)) {
  ...
  if (!stat.isFile()) {
    throw new Error(`credential KEK path is not a file: ${path}`);
  }
  if ((stat.mode & 0o077) !== 0) {
    throw new Error(`credential KEK file must be readable only by owner: ${path}`);
  }

  const encoded = readFileSync(path, 'utf8').trim();
  if (!/^[0-9a-f]{64}$/i.test(encoded)) {
    throw new Error(`credential KEK file must contain a 32-byte hex key: ${path}`);
  }
...
export function encryptEnvelopeJson(value: unknown, aad: string, kek: CredentialKek): EnvelopeCipher {
...
export function decryptEnvelopeJson<T>(cipherText: string, metaText: string, aad: string, kek: CredentialKek): T {
  ...
  if (meta.schemaVersion !== CURRENT_SCHEMA_VERSION || meta.alg !== AES_256_GCM || meta.kekId !== kek.id || meta.aad !== aad) {
    throw new Error('credential envelope metadata does not match the expected binding.');
  }
...
export function encryptSelfContainedJson(value: unknown, aad: string, kek: CredentialKek): string {
  const encrypted = encryptEnvelopeJson(value, aad, kek);
  return JSON.stringify(encrypted);
}

export function decryptSelfContainedJson<T>(value: string, aad: string, kek: CredentialKek): T {
